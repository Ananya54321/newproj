...
import { getCommunityEventRegistrations, getCommunityBySlug } from '@/lib/community/service'
import { supabaseClient } from '@/lib/supabase/client'
import type { CommunityEvent, CommunityEventRegistrationWithUser } from '@/lib/auth/types'
import { format } from 'date-fns'
import { toast } from 'sonner'

export default function CommunityEventRegistrationsPage() {
  const { slug, id } = useParams<{ slug: string; id: string }>()
  const { user } = useAuth()
  const router = useRouter()
  const [event, setEvent] = useState<CommunityEvent | null>(null)
  const [registrations, setRegistrations] = useState<CommunityEventRegistrationWithUser[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        // Fetch event and verify the current user is the creator
        ...
        const [community, regs] = await Promise.all([
          getCommunityBySlug(slug),
          getCommunityEventRegistrations(id),
        ])
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-card rounded-2xl boty-shadow">
            ...
            <p className="text-sm text-muted-foreground">
              Share this event to get community members to register.
            </p>
          </div>
        ) : (
                ...
                <li key={reg.id} className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-foreground text-sm truncate">
                      {reg.user?.full_name ?? 'Community Member'}
                    </p>
