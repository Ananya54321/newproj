...
export default function CommunityFeedPage() {
  ...
  const [userCommunities, setUserCommunities] = useState<CommunityWithMembership[]>([])
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [allPosts, allCommunities, userComms] = await Promise.all([
        getPosts({ sort, userId: user?.id }),
        getCommunities(),
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">{feedPosts.length} posts</span>
            </div>
            ...
            {trendingCommunities.length > 0 && (
              <div className="bg-card rounded-2xl p-5 boty-shadow">
                  ...
                  {trendingCommunities.map((c, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-foreground group-hover:text-primary boty-transition truncate">
                          {c.name}
                        </p>
                ...
                <Link
                  ...
                  className="text-xs text-primary hover:underline mt-4 block boty-transition"
                >
                  See all communities →
                </Link>
              </div>
