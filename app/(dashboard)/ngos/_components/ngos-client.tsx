...
interface Props {
  initialNgos: (Profile & { ngo_profile: NgoProfile })[]
  initialEvents: NgoEventWithNgo[]
}
...
export function NgosClient({ initialNgos, initialEvents }: Props) {
  ...
  const filteredEvents = initialEvents.filter((e) => eventFilter === 'all' || e.type === eventFilter)
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div className="flex gap-1 p-1 bg-card rounded-xl boty-shadow w-fit">
          {(['organizations', 'events'] as Tab[]).map((t) => (
            ...
            >
              {t === 'organizations' ? 'Organizations' : `Events${initialEvents.length ? ` (${initialEvents.length})` : ''}`}
            </button>
          ))}
        </div>
