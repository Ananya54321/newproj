...
import { updateAppointmentStatus, formatAppointmentDateTime } from '@/lib/vets/service'
...
interface Props {
  vetProfile: VeterinarianProfile | null
  initialAppointments: AppointmentWithRelations[]
}

export function VetPracticeClient({ vetProfile, initialAppointments }: Props) {
  ...
  const monthCompleted = appointments.filter(
    (a) => a.status === 'completed' && new Date(a.scheduled_at) >= monthStart
  ).length

  const tabAppointments =
    tab === 'requests' ? pending : tab === 'upcoming' ? upcoming : history
  ...
  const handleStatusChange = async (
    ...
    if (error) {
      setAppointments(snapshot) // revert on failure
      toast.error(error)
    } else {
      toast.success(
        status === 'confirmed'
...
function StatCard({
  ...
  const bg =
    color === 'amber' ? 'bg-amber-50' : color === 'green' ? 'bg-green-50' : 'bg-primary/5'
  return (
